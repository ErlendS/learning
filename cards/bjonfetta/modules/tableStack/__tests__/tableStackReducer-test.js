...
const tableStackReducer = require('../tableStackReducer');
...
test('ADD_CARDS -- completes action if state is undefined', t => {
  ...
  t.deepEqual(expected, actual)
  t.true(cards !== actual, 'has to retun a new object refrence')
})
...
test('ADD_CARDS -- can add spesefied cards', t => {
  ...
  t.deepEqual(expected, actual)
  t.true(tableStackState !== actual, 'has to return a new object reference')
})
...
test('REMOVE_CARDS -- can remove spesefied cards', t => {
  ...
  t.deepEqual(expected, actual)
  t.true(tableStackState !== actual, 'has to return a new object refrence')
})
...
test('can run multiple actions in sequence', t => {
  ...
  t.deepEqual(expected, actual)
  t.true(tableStackState !== actual, 'has to retun a new object refrence')
})
